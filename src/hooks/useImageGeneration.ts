...
import { Generation, Edit, Asset, AspectRatio, ImageSize } from '../types';
...
export const useImageEditing = () => {
  ...
  const editMutation = useMutation({
    mutationFn: async (instruction: string) => {
      ...
      const sourceImage = canvasImage || (editReferenceImages && editReferenceImages.length > 0 ? editReferenceImages[0] : null);
      if (!sourceImage) throw new Error('Нет изображения для редактирования');
